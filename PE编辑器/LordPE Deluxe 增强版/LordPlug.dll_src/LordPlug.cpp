...
void _cdecl Searchimport(const DWORD reversed, HWND hWnd)
{
	try
	{
		...
		pItem=(char*)new TCHAR [512]; 
		ZeroMemory(pItem,512);
		pstPoint=new POINT; 
		ZeroMemory(pstPoint,sizeof(POINT));
		...
		do{
			...
			plvitem->cchTextMax=512;
			plvitem->iSubItem=4; 
			plvitem->pszText=pItem;
			SendMessage(hWinList, LVM_GETITEMTEXT, (WPARAM)iItem, (LPARAM)plvitem);
		}
		while(memicmp(cSearchBuffer,pItem,nSearchLength)!=0);
...
//(2) 给LordPE查看输入表部分加右键菜单(仅复制ThunkRVA/FirstThunk列)
 ...
 void _cdecl  PopMenuCopy(const DWORD reversed, HWND hWnd)
 {
	 try
	 {
		 ...
		 { 
			 EmptyClipboard(); 
			 clipbuffer = GlobalAlloc(GMEM_DDESHARE,lstrlen(pItem)+1); 
			 if(clipbuffer==NULL) return;
			 
			 buffer = (char*)GlobalLock(clipbuffer); 
			 strcpy(buffer, LPCSTR(pItem)); 
