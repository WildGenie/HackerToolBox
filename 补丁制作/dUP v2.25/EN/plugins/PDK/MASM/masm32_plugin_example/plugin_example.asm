...
.data
example_plugin_info		PLUGIN_INFO <DUP2_PLUGIN_VERSION,"com.d2k2.plugin.example","[Plugin Example]","plugin_example.d2p">
...
DialogPlugin proc uses ebx esi edi hwnd,message,wparam,lparam
	...
	.if eax==WM_INITDIALOG
		...
		;---load window position (use the unique id)---
      		...
      		;---tool tip text---
		fn AddToolTip,hwnd,DLG_FILE,"this is the target file"
	...
	.elseif eax==WM_COMMAND
		...
		.elseif ax==BTN_LOAD
			...
			fn d2k2_GetFilePath,addr local_buffer,chr$("All Files",0,"*.*",0,"Program Files [exe,dll]",0,"*.exe;*.dll",0,0),"c:\",hDialogPlugin
			.if eax
