...
begin
  ...
  try
    ...
    if PE.LastError <> E_OK then begin
      AddInfo('Incompatible file format !!! Posible not Win32 PE image !!! ' +
              'Aborting...' + #13#10);
      Exit;
    end;
    ...
    then begin
      AddInfo('Can''t process file, coz no relocations present in image !!! ' +
              'Aborting...' + #13#10);
      Exit;
    end;
    ...
    if OriginalBase = NewBase then begin
      AddInfo('Nothing to do, coz New ImageBase is same as Original ImageBase '+
              '!!! Aborting...' + #13#10);
      Exit;
    end;
    if NewBase = 0 then begin
      AddInfo('Incompatible New ImageSize value !!! Aborting...' + #13#10);
      Exit;
    end;

    AddInfo('ReBasing ' + String(FileName) + #13#10 +
            '====================' + #13#10);

    AddInfo('Create BackUp... ');
    BackUpCreated := MakeBackUp;
    if BackUpCreated = true then Info := 'Ok.' else Info := 'Skipped.';
    AddInfo(Info + #13#10);

    AddInfo('ReBasing Image... ');
    PE.ReBasePEFile(NewBase);
    if PE.LastError = E_OK then Info := 'Ok.' else Info := 'Failed.';
    AddInfo(Info + #13#10);
    if PE.LastError <> E_OK then begin
      AddInfo(#13#10 + 'Can''t process file !!! Posible corrupt FixUp''s data '+
              '!!! Aborting...' + #13#10);
      Exit;
    end;
    ...
    AddInfo('====================' + #13#10 + 'ReBasing Done.' + #13#10 +
            'File rebased from 0x' + IntToHex8(OriginalBase) + ' to 0x' +
