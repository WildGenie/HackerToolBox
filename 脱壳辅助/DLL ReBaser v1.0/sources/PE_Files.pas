...
const
  ...
  M_NO_RELOCATIONS      =  'Relocations not found. ';
  M_OTHER_ERROR         =  'Unknown error. ';
...
type
 ...
 public
   ...
   procedure   ReBasePEFile(NewImageBase: DWord);
 private
   ...
   procedure   ProcessFixUps(hModule,pBase,RVA: Cardinal); stdcall;
 end;
...
{ +===========================================+ }
{ | ReBase Image to new ImageBase.            | }
{ +===========================================+ }
procedure PE_File.ReBasePEFile(NewImageBase : DWord);
begin
  if (PE_Header.Fix_Up_Table_RVA = 0) or (PE_Header.Fix_Up_Data_Size <= 8) then
  begin
    LastError := E_NO_RELOCATIONS;
    if ShowDebugMessages then DebugMessage(M_NO_RELOCATIONS);
    Exit;
  end;

  ProcessFixUps(DWord(pMap), DWord(pMap)+(PE_Header.Image_Base - NewImageBase),
                PE_Header.Fix_Up_Table_RVA);

  PE_Header.Image_Base := NewImageBase;
  LastError := E_OK;
end;
