library checkremotedebuggerpresent;

uses Windows;

function Enigma_Plugin_About : PWideChar;
begin
  Enigma_Plugin_About := 'Enigma anti-debugger plugin - CheckRemoteDebuggerPresent © Vladimir Sukhov 30 August 2008';
end;

function Enigma_Plugin_Description : PWideChar;
begin
  Enigma_Plugin_Description := 'This plugin uses CheckRemoteDebuggerPresent function'#10#13 +
...
var
  TimerID : cardinal;
  fn : function (hProcess : cardinal; var pbDebuggerPresent : boolean) : boolean; stdcall;
...
begin
  fn := GetProcAddress(GetModuleHandleA('kernel32.dll'), 'CheckRemoteDebuggerPresent');
  // function presents only since Windows XP SP1
  if @fn = nil then Exit;
  CheckDebugger;
