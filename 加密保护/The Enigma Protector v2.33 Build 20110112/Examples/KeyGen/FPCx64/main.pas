...
uses
  ...
  Dialogs, StdCtrls, ComCtrls, EditBtn, DateUtils;
...
type
  ...
  TfrmMain = class(TForm)
    bClose: TButton;
    dtpRegisterBefore: TDateEdit;
    dtpRegisterAfter: TDateEdit;
    dtpExpirationDate: TDateEdit;
    Label1: TLabel;
    Label2: TLabel;
    lbUserInfo: TLabel;
    eUserInfo: TEdit;
...
begin
  dtpExpirationDate.Enabled := TCheckBox(Sender).Checked;
end;

procedure TfrmMain.FormShow(Sender: TObject);
begin
  dtpExpirationDate.Date:=Today;
  dtpRegisterAfter.Date:=Today;
  dtpRegisterBefore.Date:=Today;
...
var
  kg : TKeyGenParams;
  KeyBuffer : array [0..2047] of char;
  dwResult : dword;
begin
  mKey.Clear;
  ...
  // Fill User Info
  kg.RegInfo := PAnsiChar(AnsiString(eUserInfo.Text));
  kg.RegInfoLen := strlen(PAnsiChar(kg.RegInfo));
  // Key expiration info
  kg.UseKeyExpiration := cbExpirationDate.Checked;;
  kg.ExpirationYear := YearOf(dtpExpirationDate.Date);
  kg.ExpirationMonth := MonthOf(dtpExpirationDate.Date);
  kg.ExpirationDay := DayOf(dtpExpirationDate.Date);
  // Register After date
  kg.UseRegisterAfter := cbRegisterAfter.Checked;
  if kg.UseRegisterAfter then
  begin
    kg.RegisterAfterYear := YearOf(dtpRegisterAfter.Date);
    kg.RegisterAfterMonth := MonthOf(dtpRegisterAfter.Date);
    kg.RegisterAfterDay := DayOf(dtpRegisterAfter.Date);
  end;
  // Register Before date
  kg.UseRegisterBefore := cbRegisterBefore.Checked;
  if kg.UseRegisterBefore then
  begin
    kg.RegisterBeforeYear := YearOf(dtpRegisterBefore.Date);
    kg.RegisterBeforeMonth := MonthOf(dtpRegisterBefore.Date);
    kg.RegisterBeforeDay := DayOf(dtpRegisterBefore.Date);
  end;
  // Country
  ...
  kg.HardwareID := PAnsiChar(AnsiString(eHardware.Text));
  // Crypt Sections
  ...
  begin
    mKey.Text := String(PAnsiChar(kg.Key));
  end else
  begin
    case dwResult of
...
begin
  ...
  kv.RegInfo := PAnsiChar(AnsiString(eUserInfo.Text));
  kv.RegInfoLen := strlen(PAnsiChar(kv.RegInfo));
  kv.Key := PAnsiChar(AnsiString(mKey.Text));
  kv.KeyLen := strlen(PAnsiChar(kv.Key));
  ...
  begin
    kv.HardwareID := PAnsiChar(AnsiString(eHardware.Text));
  end;
