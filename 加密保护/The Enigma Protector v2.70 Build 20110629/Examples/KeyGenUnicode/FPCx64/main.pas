...
begin
  ...
  // Fill User Info
  kg.RegInfo := PWideChar(WideString(UTF8Decode(eUserInfo.Text)));
  // Key expiration info
  ...
  kg.HardwareID := PWideChar(WideString(UTF8Decode(eHardware.Text)));
  // Crypt Sections
  ...
  begin
    mKey.Text := WideString(PWideChar(kg.Key));
  end else
  begin
    case dwResult of
...
begin
  ...
  kv.RegInfo := PWideChar(WideString(UTF8Decode(eUserInfo.Text)));
  kv.Key := PWideChar(WideString(UTF8Decode(mKey.Text)));
  ...
  begin
    kv.HardwareID := PWideChar(WideString(UTF8Decode(eHardware.Text)));
  end;
