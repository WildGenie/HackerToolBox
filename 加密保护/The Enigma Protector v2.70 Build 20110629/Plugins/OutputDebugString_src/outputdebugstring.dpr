...
begin
  Enigma_Plugin_About := 'Enigma anti-debugger plugin - OutputDebugString © Vladimir Sukhov 30 August 2008';
end;

function Enigma_Plugin_Description : PWideChar;
begin
  Enigma_Plugin_Description := 'This plugin uses OutputDebugString function to find a debugger.'#10#13 +
...
begin
  // This plugin uses OutputDebugString function to find a debugger
...
begin
  // it works only on XP! On other Os it always detects a debugger.
  lpVersionInfo.dwOSVersionInfoSize := sizeof(lpVersionInfo);
  if not GetVersionExA(lpVersionInfo) then Exit;
  if (lpVersionInfo.dwMajorVersion = 5) and (lpVersionInfo.dwMinorVersion = 2) then
  begin
    CheckDebugger;
