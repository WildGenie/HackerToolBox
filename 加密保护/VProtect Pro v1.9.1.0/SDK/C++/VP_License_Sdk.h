...
#define VP_SDK_INDEX_CHANGENOTIFY		0x10
#define VP_SDK_INDEX_CHECKLICENSELIMITE	0x11
//KeyGen Struct
#pragma pack(4) 
...
extern "C" {
	...
	__declspec(dllimport) DWORD __stdcall VP_Sdk_GetVaildDay(DWORD Sdk_Index=VP_SDK_INDEX_GETCVAILDDAY);

	/*
	* 未公开API(Undocumented Api)
	* 随时可能修改删除，仅供定制版本用户调用
	*/
	__declspec(dllimport) void __stdcall VP_Sdk_ChangeNotify(DWORD Sdk_Index,BOOL NewState);
	__declspec(dllimport) BOOL __stdcall Sdk_CheckLicenseLimite(DWORD Sdk_Index=VP_SDK_INDEX_CHECKLICENSELIMITE);
