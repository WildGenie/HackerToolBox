...
#define snew(type) ((type *)malloc(1 * sizeof(type)))
...
unsigned char *bignum_to_bytes( const Bignum bn, int *nbytes )
{
	...
	for( int i = len-1; i >= 0; --i )
		*p++ = (unsigned char)bignum_byte( bn, i );
	return ret;
}
...
int ssh1_read_bignum(const unsigned char *data, int len, Bignum * result)
{
    ...
    *result = bignum_from_bytes(p, b);
    ...
    return static_cast<int>(p + b - data);
}
...
void bignum_set_bit(Bignum bn, int bitnum, int value)
{
    if (bitnum >= (int)(BIGNUM_INT_BITS * bn[0])) abort();		       /* beyond the end */
    else {
