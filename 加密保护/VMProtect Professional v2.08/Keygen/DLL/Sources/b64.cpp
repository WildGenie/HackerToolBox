...
{
	size_t nTotal = (len + 2) * 4 / 3 + 1;
	size_t nNewLines = nTotal / 76 + 1;
	return nTotal + nNewLines;
}
...
int base64_encode(const unsigned char* bytes_to_encode, size_t in_len, char *dst) 
{
  ...
  while (in_len--) {
    ...
    if (i == 3) {
      ...
      for(i = 0; (i <4) ; i++)
	  {
        dst[len++] = base64_chars[char_array_4[i]];
		if (len % 77 == 76) {dst[len++] = '\n';}
	  }
      i = 0;
    }
  }
  ...
  if (i)
  {
    ...
    for (j = 0; (j < i + 1); j++)
	{
      dst[len++] = base64_chars[char_array_4[j]];
	  if (len % 77 == 76) {dst[len++] = '\n';}
	}
...
void base64_decode(const char *src, size_t len, std::vector<byte> &ret) 
{
  ...
  while (len-- && src[in_] != '=') 
  {
	if (!is_base64(src[in_])) {in_++; continue;} // skip non-base64 characters
