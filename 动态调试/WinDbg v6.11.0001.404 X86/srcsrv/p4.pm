...
sub GatherFileInformation {
    ...
    foreach (@Files) {
        ...
        if ( $_ =~ m/^(.*)\#(\d*)\s-\s([A-Z]:\\.*)/i ) {
            ...
            # If we're using OLDROOT and NEWROOT, translate the local path
            # (the NEWROOT) to the path we expect to lookup the file based
            # on (the OLDROOT)
            if ( defined $$self{'P4_OLDROOT'} &&
                 defined $$self{'P4_NEWROOT'} ) {
                $local_file =~ s/^\Q$$self{'P4_NEWROOT'}\E/$$self{'P4_OLDROOT'}/i;
            }
