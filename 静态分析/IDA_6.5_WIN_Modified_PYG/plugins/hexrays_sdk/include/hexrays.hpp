...
typedef qvector<uchar> byteseq_t;       ///< Byte sequence
...
/// If an invalid type is encountered, it throws \ref vd_failure_t with \ref MERR_UNKTYPE
/// \return pointer to resolved type string. The pointer is valid until a type
///         is modified in IDA kernel. The pointer might also point to the input
///         \c type if the input type was not a typedef.
/// \param type type string to resolve
/// \param fields ptr to ptr to the accompanying \link typestring fields \endlink string
...
bool hexapi is_type_integral(const type_t *type);


/// Is a small structure or union?
/// \return true if the type ia small s UDT (user defined type).
///              Small UDTs fit into a register (or pair or registers) as a rule.

bool hexapi is_type_small_struni(const type_t *ptr);
...
int hexapi partial_type_num(const type_t *type);


/// Get a type byte for a floating point value.
/// \returns BT_FLOAT or'ed with the right modifier.
/// \param width width of the desired type

type_t hexapi get_float_bit(int width);
...
typestring hexapi get_int_type_by_width_and_sign(int srcwidth, type_sign_t sign);
...
typestring hexapi get_unk_type(int size);


/// Generate a dummy pointer type
///  \param type output buffer. must be at least 3 bytes long

inline type_t *dummy_ptrtype(type_t *type, size_t size, bool isfp)
{
  type[0] = BT_PTR;
  type[1] = isfp ? get_float_bit(size) : get_unk_type_bit(size);
  type[2] = '\0';
  return type;
}
...
bool hexapi get_member_type(const member_t *mptr, typestring *type, qtype *fields=NULL);


/// Create an array type.
/// This function performs the following conversion: "type" -> "type[N]"
/// \param type array element type.
/// \param nelems number of array elements. zero represents unknown array dimensions.

typestring hexapi make_array(const typestring &type, int nelems);
...
typestring hexapi make_pointer(const typestring &type);


/// Create a reference to a named type.
/// \param name type name
/// \return type which refers to the specified name. For example, if name is "DWORD",
///             the type string which refers to "DWORD" is created.

typestring hexapi create_typedef(const char *name);


/// Get pointed object type.
/// This function performs the following conversion: "type*" -> "type"
/// \param type this parameter should be of a pointer type. The function will return
///             the pointed type. If the parameter is not a pointer, then it is
///             returned as is.

typestring hexapi remove_pointer(const typestring &type);
...
bool hexapi cnv_array_to_ptr(typestring &type);
...
inline mreg_t get_lvarloc_reg1(argloc_t location) { return is_reg2_argloc(location) ? get_argloc_r1(location) : location & ~ARGLOC_REG; }
inline mreg_t get_lvarloc_reg2(argloc_t location) { return get_argloc_r2(location); }

/// Do two arglocs overlap?
bool hexapi arglocs_overlap(argloc_t loc1, size_t w1, argloc_t loc2, size_t w2);

/// Local variable locator. Local variables are located using: definition ea, location
struct lvar_locator_t
{
  argloc_t location;    ///< Variable mapping.
                        ///< registers: ARGLOC_REG|microreg\n
                        ///< stkvars: offset from the minimal sp
  ea_t defea;           ///< Definition address. This value is assigned
                        ///< to each lvar by lvar allocator.
                        ///< BADADDR for function arguments
  lvar_locator_t(void) : location(BAD_ARGLOC), defea(BADADDR) {}
  lvar_locator_t(argloc_t loc, ea_t ea) : location(loc), defea(ea) {}
  ...
  bool is_reg_lvar(void) const { return  is_reg_argloc(location); }
  /// Is variable located on two registers?
  bool is_reg2_lvar(void) const { return  is_reg2_argloc(location); }
  /// Is variable located on the stack?
  bool is_stk_lvar(void) const { return !is_reg_argloc(location); }
  /// Get number of the register for the variable
  mreg_t get_regloc1(void) const { return get_lvarloc_reg1(location); }
  /// Get number of the second register (only for tworeg lvars)
  mreg_t get_regloc2(void) const { return get_lvarloc_reg2(location); }
...
#define CVAR_SPOILED 0x1000     ///< internal flag, do not use
//@}
...
public:
  ...
  typestring _type;      ///< variable type
  ...
  lvar_t(const qstring &n, uint32 l, ea_t e, const typestring &t, int w, int db)
    : lvar_locator_t(l, e), flags(CVAR_USED), name(n), _type(t), width(w),
      defblk(db), divisor(0) {}
  ...
  bool hexapi accepts_type(const typestring &t);
  ...
  void hexapi set_lvar_type(const typestring &t);
...
struct lvars_t : public qvector<lvar_t>
{
  ...
  int find_input_lvar(argloc_t argloc, int size) { return find_lvar(argloc, size, 0); }
  ...
  /// \return -1 if failed, otherwise the index into the variables vector.
  int hexapi find_lvar(argloc_t location, int width, int defblk=-1);
};
...
/// \return pointer to the decompilation result. It is the caller's
///         responsability to delete it. NULL if failed.

cfunc_t *hexapi decompile(func_t *pfn, hexrays_failure_t *hf);
...
/// \param vdrun_flags \ref VDRUN_

bool hexapi decompile_many(const char *outfile, eavec_t *funcaddrs, int vdrun_flags);
...
#define MERR_MAX_ERR   22
#define MERR_LOOP      (-23)   ///< internal code: redo last loop (never reported)
//@}
...
//@}
  ...
  qstring type_name;      ///< for stroffs: structure for offsetof()\n
                          ///< for enums: enum name
  /// Contructor
  number_format_t(int _opnum=0) : flags(0), opnum(char(_opnum)), props(0), org_nbytes(0) {}
  /// Get number radix
  /// \return 2,8,10, or 16
  ...
  /// Should the number be printed negated?
  /// In some cases the decompiler automatically negates numbers
  bool is_invsign(void) const { return is_signed_data(flags); }
};
...
struct cnumber_t
{
  ...
  size_t hexapi print(char *buf, size_t bufsize, const type_t *type) const;

  /// Get value.
  /// This function will properly extend the number sign to 64bits
  /// depending on the type sign.
  uint64 hexapi value(const typestring &type) const;
...
struct cfunc_parentee_t : public ctree_parentee_t
{
  ...
  /// \return false if failed
  bool hexapi calc_rvalue_type(const cexpr_t *e, typestring &target);
};
...
enum item_preciser_t
{
  ...
  ITP_SIGN = 0x40000000, // negative case value (if ITP_CASE)
  ITP_CASE = 0x80000000
};
...
struct citem_t
{
  ...
  citem_t(void) : ea(BADADDR), op(cot_empty), label_num(-1) {}
  citem_t(ctype_t o) : ea(BADADDR), op(o), label_num(-1) {}
  /// Swap two citem_t
  void swap(citem_t &r)
  {
    std::swap(ea, r.ea);
...
struct cexpr_t : public citem_t
{
  ...
  typestring type;    ///< expression type. must be carefully maintained
...
//@}
  ...
  void hexapi put_number(cfunc_t *func, uint64 v, int nbytes, type_sign_t sign=no_sign);
  ...
  bool is_nice_cond(void) const { return is_nice_expr() && is_type_bool(type[0]); }
  /// Is call object?.
  ...
  /// Get expression sign
  type_sign_t get_type_sign(void) const { return ::get_type_sign(idati, type.u_str()); }
  /// Is expression unsigned?
  bool is_type_unsigned(void) const { return ::is_type_unsigned(idati, type.u_str()); }
  /// Is expression signed?
  bool is_type_signed(void) const { return ::is_type_signed(idati, type.u_str()); }
  /// Get number of bits that can really be used by the expression.
  /// For example, x % 16 uses only 4 bits
  int hexapi get_nbits(int pbits) const;
  ...
  {
    uint64 v;
    if ( get_const_value(&v)
      && (ea_t(v) != v || !isEnabled((ea_t)v)) )
      return false;
    return true;
  }
  ...
  const cexpr_t *find_op(ctype_t op) const
  {
    ...
    return NULL;
        ...
        cexpr_t *find_op(ctype_t op) { return (cexpr_t *)((const cexpr_t *)this)->find_op(op); }
...
struct cif_t : public ceinsn_t
{
  ...
  cif_t(const cif_t &r) : ithen(NULL), ielse(NULL) { *this = r; }
...
struct cloop_t : public ceinsn_t
{
  ...
  cloop_t(const cloop_t &r) : body(NULL) { *this = r; }
...
struct cinsn_t : public citem_t
{
  ...
  /// \param ea statement address
  cinsn_t &hexapi new_insn(ea_t ea);
...
{                                       // its elements freely
  int remove_gotos(intseq_t &used_labels, int break_target, int continue_target, int next_num);
  bool use_curly_braces(void) const;
...
struct carg_t : public cexpr_t
{
  ...
  typestring formal_type;     ///< formal parameter type (if known)
...
{
  typestring functype;   ///< function object type
  carglist_t(void) {}
  carglist_t(const typestring &ftype) : functype(ftype) {}
...
AS_PRINTF(3, 0) cexpr_t *hexapi vcreate_helper(bool standalone, const typestring &type, const char *format, va_list va);

/// Create a helper object..
AS_PRINTF(3, 4) inline cexpr_t *create_helper(bool standalone, const typestring &type, const char *format, ...)
{
  va_list va;
  va_start(va, format);
...
AS_PRINTF(3, 0) cexpr_t *hexapi vcall_helper(const typestring &rettype, carglist_t *args, const char *format, va_list va);

/// Create a helper call.
AS_PRINTF(3, 4) inline cexpr_t *call_helper(
        const typestring &rettype,
        carglist_t *args,
        const char *format, ...)
{
  va_list va;
...
cexpr_t *hexapi make_num(uint64 n, type_sign_t sign=no_sign);
...
struct cfunc_t
{
  ...
  intseq_t &rgas;            ///< list of register arguments (indexes into vars)
  intseq_t &stas;            ///< list of stack arguments (indexes into vars)
...
public:
  ...
  bool hexapi get_func_type(typestring &type, qtype &fields) const;
  ...
  /// This function stores the specified in the cfunc_t structure.
  ...
  /// \param head ptr to the first item on the line (used to attach block comments). May be NULL
  /// \param item ptr to the current item. May be NULL
  /// \param tail ptr to the last item on the line (used to attach indented comments). May be NULL
  /// \sa vdui_t::get_current_item()
  bool hexapi get_line_item(const char *line, int x, bool is_ctree_line, ctree_item_t *head, ctree_item_t *item, ctree_item_t *tail);
  ...
  bool hexapi gather_derefs(int varidx, strtype_info_t *strinfo=NULL) const;
private:
  /// Cleanup.
...
enum hexrays_event_t
{
  ...
  hxe_close_pseudocode, ///< Pseudocode view has been closed.
                        ///< vdui_t *vu
...
//@}
  ...
  TCustomControl *microview; ///< unused in the current version
  TCustomControl *ct;   ///< pseudocode view

  mbl_array_t *mba;     ///< pointer to underlying microcode
  cfunc_t *cfunc;       ///< pointer to function object
  strvec_t sv;          ///< decompilation output: function text
  int hdrlines;         ///< number of lines in the declaration area
  int code;             ///< result of the last micro_request(). See \ref MERR_
  ...
  /// specified cfunc_t.
  /// \param f pointer to the function to display. The pointer will be stored
  ///          in the current vdui_t object and be destroyed when necessary.
  ///          The caller loses its ownership.
  void hexapi switch_to(cfunc_t *f);
  ...
  bool in_ctree(void) const { return cpos.in_ctree(hdrlines); }
  ...
  bool __fastcall hexapi set_lvar_type(lvar_t *v, const typestring &type);
  ...
  /// \param new_window if true, new pseudocode window will open
  bool __fastcall hexapi jump_enter(input_device_t idv, bool new_window);
  ...
  /// \return false if failed
  /// \param idv how to determine the current item
  bool __fastcall hexapi push_current_location(input_device_t idv);

  /// Pop a location from the navigation history and jump to it.
  /// \return false if failed
  bool __fastcall hexapi pop_current_location(void);
  ...
  /// \return false if failed.
  /// \param ea address of the call instruction.
  /// \param ida_argloc points past end of arglocs used by the call.
  ///                   for example, 4 means that there is one stack argument.
  bool __fastcall hexapi set_vargloc_end(ea_t ea, argloc_t ida_argloc);
...
inline const treeloc_t &user_cmts_first(user_cmts_iterator_t p)
{
...
//-------------------------------------------------------------------------
/// Get reference to the current map key
inline const operand_locator_t &user_numforms_first(user_numforms_iterator_t p)
{
...
//-------------------------------------------------------------------------
/// Get reference to the current map key
inline const citem_locator_t &user_iflags_first(user_iflags_iterator_t p)
{
...
//-------------------------------------------------------------------------
/// Get reference to the current map key
inline const int &user_labels_first(user_labels_iterator_t p)
{
...
{
  return (bool)(size_t)hexdsp(hx_is_type_correct, ptr);
}

//--------------------------------------------------------------------------
inline bool is_type_integral(const type_t *type)
{
  return (bool)(size_t)hexdsp(hx_is_type_integral, type);
}
...
//--------------------------------------------------------------------------
inline type_t get_float_bit(int width)
{
  return (type_t)(size_t)hexdsp(hx_get_float_bit, width);
}

//--------------------------------------------------------------------------
inline size_t typestring::print(char *buf, size_t bufsize) const
{
  return (size_t)hexdsp(hx_typestring_print, this, buf, bufsize);
}

//--------------------------------------------------------------------------
inline bool typestring::change_sign(type_sign_t sign)
{
  return (bool)(size_t)hexdsp(hx_typestring_change_sign, this, sign);
}

//--------------------------------------------------------------------------
inline cm_t typestring::get_cc(void) const
{
  return (cm_t)(size_t)hexdsp(hx_typestring_get_cc, this);
}

//--------------------------------------------------------------------------
inline typestring typestring::get_nth_arg(int n) const
{
  typestring retval;
  hexdsp(hx_typestring_get_nth_arg, &retval, this, n);
  return retval;
}

//--------------------------------------------------------------------------
inline typestring get_int_type_by_width_and_sign(int srcwidth, type_sign_t sign)
{
  typestring retval;
...
//--------------------------------------------------------------------------
inline typestring get_unk_type(int size)
{
  typestring retval;
...
//--------------------------------------------------------------------------
inline bool get_member_type(const member_t *mptr, typestring *type, qtype *fields)
{
  return (bool)(size_t)hexdsp(hx_get_member_type, mptr, type, fields);
}
...
//--------------------------------------------------------------------------
inline typestring remove_pointer(const typestring &type)
{
  typestring retval;
  hexdsp(hx_remove_pointer, &retval, &type);
  return retval;
}

//--------------------------------------------------------------------------
inline bool cnv_array_to_ptr(typestring &type)
{
  return (bool)(size_t)hexdsp(hx_cnv_array_to_ptr, &type);
}

//--------------------------------------------------------------------------
inline bool strtype_info_t::build_base_type(typestring *restype) const
{
  return (bool)(size_t)hexdsp(hx_strtype_info_t_build_base_type, this, restype);
}

//--------------------------------------------------------------------------
inline bool strtype_info_t::build_udt_type(typestring *restype, typestring *resfields)
{
  return (bool)(size_t)hexdsp(hx_strtype_info_t_build_udt_type, this, restype, resfields);
}

//--------------------------------------------------------------------------
inline bool arglocs_overlap(argloc_t loc1, size_t w1, argloc_t loc2, size_t w2)
{
  return (bool)(size_t)hexdsp(hx_arglocs_overlap, loc1, w1, loc2, w2);
}

//--------------------------------------------------------------------------
inline sval_t lvar_locator_t::get_regnum(void) const
{
  return (sval_t)hexdsp(hx_lvar_locator_t_get_regnum, this);
}
...
//--------------------------------------------------------------------------
inline bool lvar_t::accepts_type(const typestring &t)
{
  return (bool)(size_t)hexdsp(hx_lvar_t_accepts_type, this, &t);
}

//--------------------------------------------------------------------------
inline void lvar_t::set_lvar_type(const typestring &t)
{
...
//--------------------------------------------------------------------------
inline int lvars_t::find_lvar(argloc_t location, int width, int defblk)
{
...
{
  return (bool)(size_t)hexdsp(hx_close_pseudocode, f);
}

//--------------------------------------------------------------------------
inline cfunc_t *decompile(func_t *pfn, hexrays_failure_t *hf)
{
  return (cfunc_t *)hexdsp(hx_decompile, pfn, hf);
}

//--------------------------------------------------------------------------
inline bool decompile_many(const char *outfile, eavec_t *funcaddrs, int vdrun_flags)
{
  return (bool)(size_t)hexdsp(hx_decompile_many, outfile, funcaddrs, vdrun_flags);
}
...
//--------------------------------------------------------------------------
inline size_t cnumber_t::print(char *buf, size_t bufsize, const type_t *type) const
{
  return (size_t)hexdsp(hx_cnumber_t_print, this, buf, bufsize, type);
}

//--------------------------------------------------------------------------
inline uint64 cnumber_t::value(const typestring &type) const
{
  uint64 retval;
...
{
  return (bool)(size_t)hexdsp(hx_ctree_parentee_t_recalc_parent_types, this);
}

//--------------------------------------------------------------------------
inline bool cfunc_parentee_t::calc_rvalue_type(const cexpr_t *e, typestring &target)
{
  return (bool)(size_t)hexdsp(hx_cfunc_parentee_t_calc_rvalue_type, this, e, &target);
}
...
{
  return (bool)(size_t)hexdsp(hx_citem_t_contains_label, this);
}
...
//--------------------------------------------------------------------------
inline void cexpr_t::put_number(cfunc_t *func, uint64 v, int nbytes, type_sign_t sign)
{
  hexdsp(hx_cexpr_t_put_number, this, func, &v, nbytes, sign);
}
...
{
  return (bool)(size_t)hexdsp(hx_cexpr_t_equal_effect, this, &r);
}

//--------------------------------------------------------------------------
inline bool cexpr_t::is_child_of(const citem_t *parent) const
{
  return (bool)(size_t)hexdsp(hx_cexpr_t_is_child_of, this, parent);
}

//--------------------------------------------------------------------------
inline bool cexpr_t::contains_operator(ctype_t needed_op) const
{
  return (bool)(size_t)hexdsp(hx_cexpr_t_contains_operator, this, needed_op);
}

//--------------------------------------------------------------------------
inline int cexpr_t::get_nbits(int pbits) const
{
  return (int)hexdsp(hx_cexpr_t_get_nbits, this, pbits);
}

//--------------------------------------------------------------------------
inline bool cexpr_t::requires_lvalue(const cexpr_t *child) const
{
  return (bool)(size_t)hexdsp(hx_cexpr_t_requires_lvalue, this, child);
}

//--------------------------------------------------------------------------
inline bool cexpr_t::has_side_effects(void) const
{
  return (bool)(size_t)hexdsp(hx_cexpr_t_has_side_effects, this);
}
...
//--------------------------------------------------------------------------
inline cinsn_t &cinsn_t::new_insn(ea_t ea)
{
  return *(cinsn_t *)hexdsp(hx_cinsn_t_new_insn, this, ea);
}
...
{
  return (bool)(size_t)hexdsp(hx_cinsn_t_is_ordinary_flow, this);
}

//--------------------------------------------------------------------------
inline bool cinsn_t::contains_insn(ctype_t type) const
{
  return (bool)(size_t)hexdsp(hx_cinsn_t_contains_insn, this, type);
}

//--------------------------------------------------------------------------
inline bool cinsn_t::collect_free_breaks(cinsnptrvec_t *breaks)
{
  return (bool)(size_t)hexdsp(hx_cinsn_t_collect_free_breaks, this, breaks);
}

//--------------------------------------------------------------------------
inline bool cinsn_t::collect_free_continues(cinsnptrvec_t *continues)
{
  return (bool)(size_t)hexdsp(hx_cinsn_t_collect_free_continues, this, continues);
}
...
{
  return (ea_t)hexdsp(hx_ctree_item_t_get_ea, this);
}

//--------------------------------------------------------------------------
inline int ctree_item_t::get_label_num(void) const
{
  return (int)hexdsp(hx_ctree_item_t_get_label_num, this);
}
...
//--------------------------------------------------------------------------
inline AS_PRINTF(3, 0) cexpr_t *vcreate_helper(bool standalone, const typestring &type, const char *format, va_list va)
{
  return (cexpr_t *)hexdsp(hx_vcreate_helper, standalone, &type, format, va);
}

//--------------------------------------------------------------------------
inline AS_PRINTF(3, 0) cexpr_t *vcall_helper(const typestring &rettype, carglist_t *args, const char *format, va_list va)
{
  return (cexpr_t *)hexdsp(hx_vcall_helper, &rettype, args, format, va);
}

//--------------------------------------------------------------------------
inline cexpr_t *make_num(uint64 n, type_sign_t sign)
{
  return (cexpr_t *)hexdsp(hx_make_num, &n, sign);
}
...
//--------------------------------------------------------------------------
inline bool cfunc_t::get_func_type(typestring &type, qtype &fields) const
{
  return (bool)(size_t)hexdsp(hx_cfunc_t_get_func_type, this, &type, &fields);
}
...
{
  return (bool)(size_t)hexdsp(hx_cfunc_t_has_orphan_cmts, this);
}
...
//--------------------------------------------------------------------------
inline bool cfunc_t::get_line_item(const char *line, int x, bool is_ctree_line, ctree_item_t *head, ctree_item_t *item, ctree_item_t *tail)
{
  return (bool)(size_t)hexdsp(hx_cfunc_t_get_line_item, this, line, x, is_ctree_line, head, item, tail);
}
...
//--------------------------------------------------------------------------
inline bool cfunc_t::gather_derefs(int varidx, strtype_info_t *strinfo) const
{
  return (bool)(size_t)hexdsp(hx_cfunc_t_gather_derefs, this, varidx, strinfo);
}
...
{
  return (bool)(size_t)hexdsp(hx_install_hexrays_callback, callback, ud);
}
...
//--------------------------------------------------------------------------
inline void vdui_t::switch_to(cfunc_t *f)
{
  hexdsp(hx_vdui_t_switch_to, this, f);
}
...
{
  return (bool)(size_t)hexdsp(hx_vdui_t_refresh_cpos, this, idv);
}

//--------------------------------------------------------------------------
inline bool __fastcall vdui_t::get_current_item(input_device_t idv)
{
  return (bool)(size_t)hexdsp(hx_vdui_t_get_current_item, this, idv);
}

//--------------------------------------------------------------------------
inline bool __fastcall vdui_t::ui_rename_lvar(lvar_t *v)
{
  return (bool)(size_t)hexdsp(hx_vdui_t_ui_rename_lvar, this, v);
}

//--------------------------------------------------------------------------
inline bool __fastcall vdui_t::rename_lvar(lvar_t *v, const char *name, bool is_user_name)
{
  return (bool)(size_t)hexdsp(hx_vdui_t_rename_lvar, this, v, name, is_user_name);
}

//--------------------------------------------------------------------------
inline bool __fastcall vdui_t::ui_set_lvar_type(lvar_t *v)
{
  return (bool)(size_t)hexdsp(hx_vdui_t_ui_set_lvar_type, this, v);
}

//--------------------------------------------------------------------------
inline bool __fastcall vdui_t::set_lvar_type(lvar_t *v, const typestring &type)
{
  return (bool)(size_t)hexdsp(hx_vdui_t_set_lvar_type, this, v, &type);
}

//--------------------------------------------------------------------------
inline bool __fastcall vdui_t::ui_edit_lvar_cmt(lvar_t *v)
{
  return (bool)(size_t)hexdsp(hx_vdui_t_ui_edit_lvar_cmt, this, v);
}

//--------------------------------------------------------------------------
inline bool __fastcall vdui_t::set_lvar_cmt(lvar_t *v, const char *cmt)
{
  return (bool)(size_t)hexdsp(hx_vdui_t_set_lvar_cmt, this, v, cmt);
}

//--------------------------------------------------------------------------
inline bool __fastcall vdui_t::set_strmem_type(struc_t *sptr, member_t *mptr)
{
  return (bool)(size_t)hexdsp(hx_vdui_t_set_strmem_type, this, sptr, mptr);
}

//--------------------------------------------------------------------------
inline bool __fastcall vdui_t::rename_strmem(struc_t *sptr, member_t *mptr)
{
  return (bool)(size_t)hexdsp(hx_vdui_t_rename_strmem, this, sptr, mptr);
}

//--------------------------------------------------------------------------
inline bool __fastcall vdui_t::set_global_type(ea_t ea)
{
  return (bool)(size_t)hexdsp(hx_vdui_t_set_global_type, this, ea);
}

//--------------------------------------------------------------------------
inline bool __fastcall vdui_t::rename_global(ea_t ea)
{
  return (bool)(size_t)hexdsp(hx_vdui_t_rename_global, this, ea);
}

//--------------------------------------------------------------------------
inline bool __fastcall vdui_t::rename_label(int label)
{
  return (bool)(size_t)hexdsp(hx_vdui_t_rename_label, this, label);
}

//--------------------------------------------------------------------------
inline bool __fastcall vdui_t::jump_enter(input_device_t idv, bool new_window)
{
  return (bool)(size_t)hexdsp(hx_vdui_t_jump_enter, this, idv, new_window);
}

//--------------------------------------------------------------------------
inline bool __fastcall vdui_t::ctree_to_disasm(void)
{
  return (bool)(size_t)hexdsp(hx_vdui_t_ctree_to_disasm, this);
}

//--------------------------------------------------------------------------
inline bool __fastcall vdui_t::push_current_location(input_device_t idv)
{
  return (bool)(size_t)hexdsp(hx_vdui_t_push_current_location, this, idv);
}

//--------------------------------------------------------------------------
inline bool __fastcall vdui_t::pop_current_location(void)
{
  return (bool)(size_t)hexdsp(hx_vdui_t_pop_current_location, this);
}
...
{
  return (bool)(size_t)hexdsp(hx_vdui_t_edit_cmt, this, &loc);
}

//--------------------------------------------------------------------------
inline bool __fastcall vdui_t::edit_func_cmt(void)
{
  return (bool)(size_t)hexdsp(hx_vdui_t_edit_func_cmt, this);
}

//--------------------------------------------------------------------------
inline bool __fastcall vdui_t::del_orphan_cmts(void)
{
  return (bool)(size_t)hexdsp(hx_vdui_t_del_orphan_cmts, this);
}

//--------------------------------------------------------------------------
inline bool __fastcall vdui_t::set_num_radix(int base)
{
  return (bool)(size_t)hexdsp(hx_vdui_t_set_num_radix, this, base);
}

//--------------------------------------------------------------------------
inline bool __fastcall vdui_t::set_num_enum(void)
{
  return (bool)(size_t)hexdsp(hx_vdui_t_set_num_enum, this);
}

//--------------------------------------------------------------------------
inline bool __fastcall vdui_t::set_num_stroff(void)
{
  return (bool)(size_t)hexdsp(hx_vdui_t_set_num_stroff, this);
}

//--------------------------------------------------------------------------
inline bool __fastcall vdui_t::invert_sign(void)
{
  return (bool)(size_t)hexdsp(hx_vdui_t_invert_sign, this);
}

//--------------------------------------------------------------------------
inline bool __fastcall vdui_t::collapse_item(bool hide)
{
  return (bool)(size_t)hexdsp(hx_vdui_t_collapse_item, this, hide);
}

//--------------------------------------------------------------------------
inline bool __fastcall vdui_t::split_item(bool split)
{
  return (bool)(size_t)hexdsp(hx_vdui_t_split_item, this, split);
}

//--------------------------------------------------------------------------
inline bool __fastcall vdui_t::set_vargloc_end(ea_t ea, argloc_t ida_argloc)
{
  return (bool)(size_t)hexdsp(hx_vdui_t_set_vargloc_end, this, ea, ida_argloc);
}
